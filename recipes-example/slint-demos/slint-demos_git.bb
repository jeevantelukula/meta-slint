...
# Skia source tarball - downloaded by bitbake's fetcher (proxy/mirror aware).
# skia-bindings 0.90.0 needs tag m142-0.89.1 (commit d6b5e2f867...) from rust-skia/skia.
# We fetch by commit hash since the tag URL may 404 on codeload.github.com.
...
# Skip QA check for GitHub archive URL and build paths embedded by Rust/Skia
...
# Remove --frozen since we use CARGO_DISABLE_BITBAKE_VENDORING with network access,
# and add -p slint with feature flags (EXTRA_CARGO_FLAGS/CARGO_FEATURES from cargo_bin
# are not available in oe-core cargo.bbclass)
...
# Increase cargo HTTP timeout for unreliable network connections
...
# Prepare Skia source with all third-party dependencies.
# skia-bindings' built-in download fails in Yocto master's build environment:
#   1) The codeload.github.com tarball URL returns 404
#   2) Python urllib in fetch-gn can't resolve DNS
# So we download Skia via bitbake's fetcher, run git-sync-deps ourselves
# (git operations work fine), and set SKIA_SOURCE_DIR for offline mode.
do_configure:append() {
...
#!/usr/bin/env python3
# Replaced by Yocto recipe - gn provided by gn-native
import sys
sys.exit(0)
FETCHGN
        ...
        # git operations work fine in bitbake's build environment.
        cd ${UNPACKDIR}
...
do_compile:prepend() {
    ...
    # Tell skia-bindings to use pre-prepared Skia source (triggers offline mode,
    # skipping all internal downloads and git-sync-deps)
